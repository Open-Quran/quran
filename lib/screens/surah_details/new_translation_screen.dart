...
class NewTranslationScreen extends StatefulWidget {
  const NewTranslationScreen({Key? key}) : super(key: key);

  @override
  State<NewTranslationScreen> createState() => _NewTranslationScreenState();
}

class _NewTranslationScreenState extends State<NewTranslationScreen> {
  ...
  void scrollListener() {
    ...
    context.read<NewSurahDetailsProvider>().listenToTranslationScreenList(index);
  }
  ...
  VerseCard buildVerseCard(int index, VerseModel verse, BuildContext context) {
    return VerseCard(
      ...
      playFunction: (verse, isPlaying) {
        context.read<NewSurahDetailsProvider>().onTapVerseCardPlayOrPause(
              index,
              isPlaying,
            );
      },
      ...
      shareFunction: (verseModel) {
        context.read<NewSurahDetailsProvider>().shareVerse(verseModel, index);
      },
      selectedVerseKey: context.watch<NewSurahDetailsProvider>().selectedVerseKey,
      changeSelectedVerseKey: context.read<NewSurahDetailsProvider>().changeSelectedVerseKey,
    );
  }
}
